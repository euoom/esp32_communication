...
echo.
echo === ESP32 MicroPython Bluetooth Receiver Setup ===
echo.
...
REM 파일 전송 (receiver.py를 main.py로 업로드)
mpremote connect COM6 cp receiver.py :main.py
if errorlevel 1 (
    if !RETRY_COUNT! lss !MAX_RETRIES! (
        echo Upload failed, retrying after reset...
        if !RETRY_COUNT! equ 1 (
            echo Using soft reset for first retry...
            mpremote connect COM6 soft-reset
        ) else (
            echo Using hard reset for final attempt...
            mpremote connect COM6 reset
        )
        timeout /t 2 /nobreak > nul
        goto RETRY_UPLOAD
