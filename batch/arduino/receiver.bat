...
:: 명령줄 인자 확인
if "%~1"=="" (
    echo 사용법: receiver_arduino.bat [ino파일경로]
    echo 예시: receiver_arduino.bat ..\hello_world\arduino_bluetooth\receiver\receiver.ino
    exit /b 1
)
...
arduino-cli upload -p %RECEIVER_PORT% --fqbn %BOARD_TYPE% --verbose "!INO_FILE!"
if errorlevel 1 (
    echo 업로드 실패!
    pause
    exit /b 1
